...
4. Create initial schema migrations for users, conversations, messages
...
1. Extend `AppUser` entity with authentication fields
2. Implement password hashing (BCrypt)
3. Create JWT token provider (access + refresh tokens)
4. Implement authentication service (register, login, refresh)
5. Create Spring Security configuration
6. Add JWT authentication filter
7. Create authentication REST endpoints
...
1. Refactor `Conversation` to support multiple participants
2. Create `ConversationParticipant` join entity
3. Add group conversation support (max 50 members validation)
4. Add idempotency key to messages
5. Update repositories for new relationships
...
1. Configure WebSocket endpoint with authentication
2. Implement JWT authentication for WebSocket connections
3. Create event service for message distribution
4. Add event ID tracking for resume capability
5. Implement WebSocket message handler
6. Update message service to publish events
7. Add cursor pagination to message listing endpoint
...
1. Add `/api/v1` versioning to all endpoints
2. Create proper DTOs for requests/responses
3. Implement global exception handler
4. Add comprehensive input validation
5. Implement cursor pagination
6. Add idempotency handling for message creation
...
2. Add Micrometer metrics (message counts, latency, WebSocket connections)
3. Set up distributed tracing (OpenTelemetry)
4. Add correlation IDs for request tracking
5. Create health check endpoints with detailed status
